...
const Rewards = () => {
    ...
    const [deleteProductByCategoryId, setDeleteProductByCategoryId] = useState(""); 
    ...
    useEffect(() => {
        const fetchRestaurants = async () => {
            try {
                ...
                if (restaurantOwned) {
                    const imageUrl = "http://localhost:8080/api/restaurants/files/" + restaurantOwned.coverImageUrl;
...
{loading ? (
        <h1>Loading restaurant data. Please wait...</h1>
      ) : (
        <div>
  <section className="relative block h-500-px">
    <div className="w-full h-full bg-center bg-cover" style={backgroundStyle}>
        ...
        <div className="px-6">
          <div className="flex flex-wrap justify-center">
            <div className="w-full lg:w-3/12 px-4 lg:order-2 flex justify-center">
              <div className="relative">
                {restaurantOwned ? 
                (<img alt="..." src={"http://localhost:8080/api/restaurants/files/"+restaurantOwned.logoUrl} className="shadow-xl rounded-full h-auto align-middle border-none absolute -m-16 -ml-20 lg:-ml-16 max-w-150-px"/>)
                :(<img alt="..." src={logoNoResto} className="shadow-xl rounded-full h-auto align-middle border-none absolute -m-16 -ml-20 lg:-ml-16 max-w-150-px"/>)}
                </div>
            </div>
            ...
            <div className="w-full lg:w-4/12 px-4 lg:order-3 lg:text-right lg:self-center">
              <div className="py-6 px-3 mt-32 sm:mt-0">
                <button className="bg-pink-500 active:bg-pink-600 uppercase text-white font-bold hover:shadow-md shadow text-xs px-4 py-2 rounded outline-none focus:outline-none sm:mr-2 mb-1 ease-linear transition-all duration-150" onClick={toggleCategories} type="button">
                  Show more
                </button>
              </div>
            </div>
            ...
            <div className="w-full lg:w-4/12 px-4 lg:order-1">
              <div className="flex justify-center py-4 lg:pt-4 pt-8">
                <div className="mr-4 p-3 text-center">
                  <span className="text-xl font-bold block uppercase tracking-wide text-blueGray-600">{restaurantOwned.rating? restaurantOwned.rating:"None"}</span><span className="text-sm text-blueGray-400">Rating</span>
                </div>
                <div className="mr-4 p-3 text-center">
                  <span className="text-xl font-bold block uppercase tracking-wide text-blueGray-600">{restaurantOwned.likes?restaurantOwned.likes:"None"}</span><span className="text-sm text-blueGray-400">Likes</span>
                </div>
                <div className="lg:mr-4 p-3 text-center">
                  <span className="text-xl font-bold block uppercase tracking-wide text-blueGray-600">{restaurantOwned.priceRange?restaurantOwned.priceRange:"None"}</span><span className="text-sm text-blueGray-400">Price Range</span>
                </div>
              </div>
            </div>
          </div>
          <div className="text-center mt-12">
            ...
            <div className="mb-2 text-blueGray-600 mt-10">
              <i className="fas fa-briefcase mr-2 text-lg text-blueGray-400"></i>{/*Solution Manager - Creative Tim Officer*/}{restaurantOwned.cuisine?restaurantOwned.cuisine:"No cuisine"}
            </div>
            <div className="mb-2 text-blueGray-600">
              <i className="fas fa-university mr-2 text-lg text-blueGray-400"></i>{/*University of Computer Science*/}{restaurantOwned.phoneNumber?restaurantOwned.phoneNumber:"No phone number"}
            </div>
          </div>
            ...
            <div class="flex flex-wrap justify-center">
              <div class="w-full lg:w-9/12 px-4">
                <p class="mb-4 text-lg leading-relaxed text-blueGray-700">
                    Description : {restaurantOwned.description?restaurantOwned.description:"None"}
                </p>
                <a href="#pablo" onClick={toggleCategories} class="font-normal text-pink-500">Show more</a>
              </div>
            </div>
          </div>
    ...
    <div class="flex flex-wrap -m-4">
      ...
      <div class="lg:w-1/4 md:w-1/2 p-4 w-full">
        <a class="block relative h-48 rounded overflow-hidden">
          <img alt="ecommerce" class="object-cover object-center w-full h-full block" src={"http://localhost:8080/api/product/files/"+product.img}/>
        </a>
        <div class="mt-4">

...
const DarkModeButton = ({ dark, onToggle }) => (
  <ul className="ml-6">
    <li>
      <button
        type="button"
...
const Navbar = () => {
  ...
  // Derive role booleans directly from currentUser — no state needed
  const showModeratorBoard = currentUser?.roles.includes('ROLE_MODERATOR') ?? false;
  const showAdminBoard = currentUser?.roles.includes('ROLE_ADMIN') ?? false;
  ...
  return (
    <nav className="bg-quartenary sticky top-0 z-10 border-b border-gray-200 dark:bg-gray-900">
      <div className="flex flex-wrap items-center justify-between max-w-screen-xl mx-auto p-4">
        <a href="/" className="flex items-center space-x-3 rtl:space-x-reverse">
          <svg
            className="css-ze2te4 css-1j6mmh7 h-7"
          ...
          <Link
            to={'/'}
            className="dark:text-white self-center text-xl font-semibold whitespace-nowrap"
          >
            Coffee & Plates
          </Link>
        </a>
        ...
        <div className="flex items-center md:order-2 space-x-1 md:space-x-2 rtl:space-x-reverse">
          {currentUser ? (
            <div className="text-sm">
              <Link
                to={'/profile'}
                className="dark:text-white text-gray-800 hover:bg-gray-50 focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm px-4 py-2 md:px-5 md:py-2.5 dark:hover:bg-gray-700 focus:outline-none dark:focus:ring-gray-800"
              >
            ...
            <div className="text-white text-sm mr-4">
              <Link
                to={'/login'}
                className="dark:text-white mr-2 text-gray-800 hover:bg-gray-100 focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm px-4 py-2 md:px-5 md:py-2.5 dark:hover:bg-gray-700 focus:outline-none dark:focus:ring-gray-800"
              >
                Login
              </Link>
              <Link
          ...
          <DarkModeButton dark={dark} onToggle={darkModeHandler} />
        ...
        >
          <ul className="flex flex-col md:flex-row md:space-x-4">
            <li>
              <Link to={'/home'} className="hover:text-gray-300 dark:text-white">
                Home
              </Link>
            </li>
            {currentUser && (
              <li>
                <Link to={'/restaurant/new'} className="hover:text-gray-300 dark:text-white">
                  Restaurant
                </Link>
              </li>
            )}
            {currentUser && (
              <li>
                <Link to={'/menu'} className="hover:text-gray-300 dark:text-white">
                  Menu
                </Link>
              </li>
            )}
            {currentUser && (
              <li>
                <Link to={'/rewards'} className="hover:text-gray-300 dark:text-white">
                  Rewards & Discounts
                </Link>
              </li>
            )}
            {currentUser && (
              <li>
                <Link to={'/dashboard'} className="hover:text-gray-300 dark:text-white">
                  Dashboard
                </Link>
              </li>
            )}
            {showModeratorBoard && (
              <li>
                <Link to={'/mod'} className="hover:text-gray-300 dark:text-white">
                  Moderator
                </Link>
              </li>
            )}
            {showAdminBoard && (
              <li>
                <Link to={'/admin'} className="hover:text-gray-300 dark:text-white">
                  Admin
                </Link>
              </li>
            )}
